...
1. **ACC يرفع صورة شهادة** موجودة لديه
2. **AI (Gemini) يحلل الصورة** ويستخرج:
   - الألوان (الخلفية، الحدود، النصوص)
   - الخطوط وأحجامها
   - مواضع العناصر
   - النصوص الثابتة
   - التخطيط العام
3. **AI يولد Template كامل** يتضمن:
   - `template_config` (JSON) يحتوي على جميع الإعدادات
   - `template_html` (HTML) جاهز للاستخدام
4. **يتم حفظ Template** في قاعدة البيانات
5. **Training Center يستخدم Template** ويدخل المتغيرات لتوليد PDF
